...
- `site-perf-page/assets/perf_updater.php` : updater côté site (clé publique embarquée, périmètre
...
- `site-perf-page/assets/perf_run.php` : point d'entrée du cron = updater best-effort puis génération.
...
> ⚠️ **Source de ces fichiers = le skill `site-perf-page/assets/`** (`perf_updater.php`, `perf_run.php`,
> `perf_gen.php`), PAS `install/` de ce repo (le back-office et le module perf ont des installeurs
> distincts). Le skill dépose les 3 fichiers et câble le cron sur `perf_run.php` **par défaut** depuis
> l'incident du 2026-07-15 (5 sites gelés car leur cron appelait `perf_gen.php` en direct → jamais
> d'auto-update ; symptôme : le panneau les affiche « en retard »).

1. Déposer `perf_updater.php` et `perf_run.php` (assets du skill) dans le `private/`
   du site (là où vivent déjà `perf_gen.php` / `perf_config.php`).
2. Le cron cPanel DOIT appeler `php …/perf_run.php` (jamais `perf_gen.php` en direct).
